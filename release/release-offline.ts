...
import { versionRequirements } from "./src/version-helpers";
import  {
...
if (!isValidVersionString(version)) {
  error('You must provide a valid version string as the first argument (e.g v0.45.6');
}
...
async function checkJar() {
  ...
  const versionProperties = (await $`jar xf ${JAR_PATH}/metabase.jar version.properties && cat version.properties`).toString();
...
async function build () {
  ...
  if (!nodeVersion.includes(`v${versionRequirements[majorVersion].node}`)) {
    error(`Node version must be v${versionRequirements[majorVersion].node}, you are running ${nodeVersion}`);
  }

  if (!javaVersion.includes(`openjdk version "${versionRequirements[majorVersion].java}`)) {
    error(`Java version must be ${versionRequirements[majorVersion].java}, you are running ${javaVersion}`);
  }
  ...
  try {
    ...
    // build jar
    await $`../bin/build.sh :edition :${edition} :version ${version}`.pipe(process.stdout);
...
async function s3 () {
  ...
  if (!AWS_S3_DOWNLOADS_BUCKET || !AWS_S3_STATIC_BUCKET || !AWS_S3_RELEASE_SECRET_ACCESS_KEY || !AWS_S3_RELEASE_ACCESS_KEY_ID) {
  ...
  const versionPath = edition === 'ee'
    ? `enterprise/${version}`
    : version;

  console.log(chalk.blue(`\n⏳ Publishing ${version} to s3\n`));

  await $`aws s3 cp ${JAR_PATH}/metabase.jar s3://${ AWS_S3_DOWNLOADS_BUCKET }/${ versionPath }/metabase.jar`.pipe(process.stdout);

  if (isLatest) {
    await $`aws s3 cp ${JAR_PATH}/metabase.jar s3://${ AWS_S3_DOWNLOADS_BUCKET }/latest/metabase.jar`.pipe(process.stdout);
  }
...
async function versionInfo() {
  ...
  const versionInfoName = edition === 'ee'
    ? 'version-info-ee.json'
    : 'version-info.json';

  fs.writeFileSync(versionInfoName, JSON.stringify(newVersionInfo, null, 2));

  await $`aws s3 cp ${versionInfoName} s3://${ AWS_S3_STATIC_BUCKET }/${ versionInfoName }`.pipe(process.stdout);
...
async function releaseNotes() {
  ...
  const checksum = (await $`shasum -a 256 ${JAR_PATH}/metabase.jar`).toString().split(' ')[0];
...
(async () => {
  ...
  if(!step) {
    console.log(chalk.red('You must provide a step argument like --build or --publish'));
  }
