// should always be in the format v1.2.3
export const normalizeVersionNumber = (
  versionString: string,
  isEnterprise = false,
) => {
  return (
    (isEnterprise ? "v1." : "v0.") +
    versionString.replace(/^(v0|v1|vx|v|x|1|0)\.?/i, "")
  );
};

// must be in the format v0.75.2.3 or v1.75.2.3
...
export const isLatestVersion = (thisVersion: string, allVersions: string[]) => {
  ...
  return getOSSVersion(thisVersion) >= lastVersion;
};
